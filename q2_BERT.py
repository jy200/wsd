...
def gather_sense_vectors(corpus: T.List[T.List[WSDToken]],
    ...
    * That yields a single vector associated to each synset; return this as
    ...
    """
    syn_dict = defaultdict(list)    # temp dictionary to append vectors to list
    corpus = sorted(corpus, key=len)
    for batch_n in trange(0, len(corpus), batch_size, desc='gathering', leave=False):
        batch = corpus[batch_n:batch_n + batch_size]
        bert_batch = list()

        # pre-process batch token lists for run_bert
        for word_tokens in batch:
            bert_batch.append([word.wordform for word in word_tokens])

        result, offsets = run_bert(bert_batch)

        for b_id, word_tokens in enumerate(batch):
            b_length = result[b_id].shape[0]
            indices_list = defaultdict(list)    # batch token indices
            look_ahead(indices_list, list(range(b_length)), offsets[b_id])
            for i, token in enumerate(word_tokens):
                if not token.synsets:
                    continue
                token_indices = indices_list[i + 1]
                vector = torch.zeros(768)
                for t in token_indices:
                    vector += result[b_id][t]
                length = len(token_indices)
                if length > 1:     # take mean if multiple offset vectors
                    vector /= length
                syn_dict[next(iter(token.synsets))].append(vector)
    ret_dict = defaultdict(Tensor)   # defaultdict of Tensors instead of lists
    for word in syn_dict.keys():
        length = len(syn_dict[word])
        vector = syn_dict[word][0]
        if len(syn_dict[word]) > 1:     # take mean if multiple vectors in list
            for vec in syn_dict[word][1:]:
                vector += vec
            vector /= length
            ret_dict[word] = vector
        else:
            ret_dict[word] = vector
    return ret_dict


def look_ahead(a_dict, token, off):
    """
    :param a_dict: Dictionary to store list of indices for each word
    :param token: batch bert_run tokens
    :param off: batch offsets
    :return: nothing: we just mutate a_dict
    """
    # Index starts from 0. word 1 = tokens[0]
    word_count = 0
    for tok, offset in zip(token, off):
        s, e = offset
        if (s, e) == (0, 0):    # skip padding/CLR/etc
            continue
        if s != 0:
            a_dict[word_count].append(tok)
        else:   # new word
            word_count += 1
            a_dict[word_count].append(tok)
...
def bert_1nn(batch: T.List[T.List[WSDToken]],
    ...
    **IMPORTANT**: When computing the cosine similarities and finding the sense
    ...
    for b_id, word_tokens in enumerate(batch):
        s_predict = list()
        target_index = indices[b_id]

        # match token indices with offsets for each token
        b_length = result[b_id].shape[0]
        indices_list = defaultdict(list)
        look_ahead(indices_list, list(range(b_length)), offsets[b_id])

        for i, token in enumerate(word_tokens):
            if i not in target_index:   # only find predictions for targets
                continue
            token_indices = indices_list[i + 1]
            vector1 = torch.zeros(768)
            for t in token_indices:     # align token parts with offset parts
                vector1 += result[b_id][t]
            length = len(token_indices)
            if length > 1:
                vector1 /= length

            # Compare similarity of possible senses
            best_sense = None
            best_score = 0
            synsets = wn.synsets(token.lemma)
            synset_list = [x for x in synsets]
            for synset in synset_list:
                name = synset.name()        # Get synset string name
                if name not in sense_vectors:   # Not in our model
                    continue
                vector2 = sense_vectors[name]
                dot = torch.matmul(vector1, vector2)
                norms = norm(vector1) * norm(vector2)
                if norms == 0:
                    continue
                score = dot / norms     # Cosine Similarity
                if score > best_score:
                    best_score = score
                    best_sense = synset
            if best_score == 0:
                s_predict.append(mfs([token], 0))
            else:
                s_predict.append(best_sense)
        predictions.append(s_predict)
    return predictions
